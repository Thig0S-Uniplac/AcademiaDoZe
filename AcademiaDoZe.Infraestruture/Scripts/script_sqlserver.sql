...
IF OBJECT_ID(N'dbo.tb_logradouro', N'U') IS NULL
    BEGIN
        CREATE TABLE tb_logradouro (
            ...
            pais          VARCHAR (50)  CONSTRAINT DF_tb_logradouro_pais DEFAULT 'Brasil' NOT NULL,
            CONSTRAINT PK_tb_logradouro PRIMARY KEY (id_logradouro),
            CONSTRAINT UQ_tb_logradouro_cep UNIQUE (cep)
        );
        CREATE INDEX IX_tb_logradouro_cep
            ON tb_logradouro(cep);
        CREATE INDEX IX_tb_logradouro_cidade
            ON tb_logradouro(cidade);
    END
...
IF OBJECT_ID(N'dbo.tb_aluno', N'U') IS NULL
    BEGIN
        CREATE TABLE tb_aluno (
            ...
            CONSTRAINT FK_tb_aluno_tb_logradouro FOREIGN KEY (logradouro_id) REFERENCES tb_logradouro (id_logradouro)
        );
        CREATE INDEX IX_tb_aluno_cpf
            ON tb_aluno(cpf);
    END
...
IF OBJECT_ID(N'dbo.tb_colaborador', N'U') IS NULL
    BEGIN
        CREATE TABLE tb_colaborador (
            ...
            CONSTRAINT FK_tb_colaborador_tb_logradouro FOREIGN KEY (logradouro_id) REFERENCES tb_logradouro (id_logradouro)
        );
        CREATE INDEX IX_tb_colaborador_cpf
            ON tb_colaborador(cpf);
    END
...
IF OBJECT_ID(N'dbo.tb_matricula', N'U') IS NULL
    BEGIN
        CREATE TABLE tb_matricula (
            ...
            restricao_medica INT             CONSTRAINT DF_tb_matricula_restricao DEFAULT 0 NOT NULL, -- Enum [Flags] MatriculaRestricoes
            obs_restricao    VARCHAR (500)   NULL,
            laudo_medico     VARBINARY (MAX) NULL,
            CONSTRAINT PK_tb_matricula PRIMARY KEY (id_matricula),
            CONSTRAINT FK_tb_matricula_tb_aluno FOREIGN KEY (aluno_id) REFERENCES tb_aluno (id_aluno) ON DELETE CASCADE
        );
        CREATE INDEX IX_tb_matricula_aluno_id
            ON tb_matricula(aluno_id);
        CREATE INDEX IX_tb_matricula_data_fim
            ON tb_matricula(data_fim);
    END
...
IF OBJECT_ID(N'dbo.tb_acesso', N'U') IS NULL
    BEGIN
        CREATE TABLE tb_acesso (
            ...
            data_hora   DATETIME CONSTRAINT DF_tb_acesso_data_hora DEFAULT GETDATE() NOT NULL,
            CONSTRAINT PK_tb_acesso PRIMARY KEY (id_acesso)
        );
        CREATE INDEX IX_tb_acesso_pessoa
            ON tb_acesso(pessoa_tipo, pessoa_id);
        CREATE INDEX IX_tb_acesso_data_hora
